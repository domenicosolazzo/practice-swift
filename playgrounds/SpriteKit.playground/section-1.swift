// Playground SpriteKit
import Cocoa
import SpriteKit
import XCPlayground


// Playground 1 (Source: http://goo.gl/qyH8eq)
class MyScene:SKScene{
    var _previousTime = 0.0
    var _deltaTime = 0.0
    
    override func didMove(to view: SKView) {
        let sprite = SKSpriteNode(color:NSColor.green, size: CGSize(width:50.0, height:50.0))
        sprite.name = "block"
        sprite.position = CGPoint(x:50.0, y:50.0)
        self.addChild(sprite)
    }
    
    override func update(_ currentTime: TimeInterval) {
        if _previousTime == 0.0 { _previousTime = currentTime}
        // Delta
        _deltaTime = currentTime - _previousTime
        
        _previousTime = currentTime
        // Retrieve the sprite by name
        var block = self.childNode(withName: "block")
        // Update block position
        block?.position = CGPoint(x: Double(block?.position.x) + Double((20.0 * _deltaTime)), y: Double(block.position.y) + Double((20.0 * _deltaTime)))
    }
    
    
}

// Create the view
var view = SKView(frame:NSRect(x:0, y:0, width:1024, height:768))
XCPShowView(identifier: "view", view: view)

// Create a new scene
var scene = MyScene(size: CGSize(width:1024, height:768))
// Present the scene
view.presentScene(scene)
// Change background color
scene.backgroundColor = NSColor.blackColor()

